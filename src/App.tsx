      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white p-6 sm:p-8 rounded-2xl shadow-xl border border-slate-100">
          <div className="text-center mb-6">
            <div className="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white shadow-md shadow-indigo-200 mx-auto mb-3">
              <Zap className="w-6 h-6 fill-white" />
            </div>
            <h1 className="text-2xl font-bold text-slate-900">NavFOR</h1>
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl mb-5 text-xs font-semibold">
            <button
              ...
              className={cn(
                ...
                !showEmailForm ? "bg-white text-slate-900 shadow-xs" : "text-slate-500 hover:text-slate-800"
              )}
            >
              Google ログイン
            </button>
            <button
              ...
              className={cn(
                "flex-1 py-2 rounded-lg transition-all text-center flex items-center justify-center gap-1",
                showEmailForm ? "bg-white text-slate-900 shadow-xs" : "text-slate-500 hover:text-slate-800"
              )}
            >
          ...
          {!showEmailForm ? (
            <div className="space-y-4">
              <p className="text-xs text-slate-500 text-center">
                通常環境ではワンタップでログインできるGoogle認証をご利用いただけます。
              </p>
              <button
                onClick={() => signIn()}
                type="button"
                className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-medium py-3 px-4 rounded-xl transition-all shadow-xs text-sm"
              >
            ...
            <form onSubmit={handleEmailAuthSubmit} className="space-y-3.5">
              <p className="text-[11px] text-amber-700 bg-amber-50 p-2.5 rounded-xl border border-amber-100">
                学内Wi-Fi（Sorbonne等）でポップアップがブロックされる場合はこちらをご使用ください。
              </p>
              <div>
              ...
              <button
                ...
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2.5 px-4 rounded-xl text-sm transition-colors shadow-xs"
              >
                {isSubmittingAuth ? '処理中...' : isSignUpMode ? '新規アカウント登録' : 'メールでログイン'}
              </button>
              ...
              <div className="text-center pt-1">
                ...
                >
                  {isSignUpMode ? '既存アカウントでログイン' : '初めての方（新規メール登録）はこちら'}
                </button>
              </div>
            </form>
          )}
